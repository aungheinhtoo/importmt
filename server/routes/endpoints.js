...
router.get("/listofpatients", authorize, async (req, res) => {
  try {
    ...
    /*const doctor_name = await pool.query(
      "SELECT * FROM users WHERE accessor = $1",
      [req.user]
    );*/

    //doctor = doctor_name.rows[0].user_name
...
router.get("/userattempts/:user_to_check", authorize, async (req, res) => {
  try {
    if (req.user.id != req.params.user_to_check) {
...
router.get("/patientattempts/:patient_id", authorize, async (req, res) => {
  try {
    ...
    if (!accessor_rights.rows[0].is_accessor) {
      ...
      const allUserAttempts = await pool.query(
        "SELECT a.attempt_no,a.attempted_on,a.time_taken,a.accuracy,a.difficulty,a.pass_fail FROM attempts a INNER JOIN users u ON u.user_id = a.user_id  WHERE u.user_name =$1 AND u.accessor = $2",
        [req.params.patient_id, req.user.name]
      );
      res.json(allUserAttempts.rows);
    }
  } catch (err) {
